...
int main(int argc, char **argv) {
    ...
    vector geon(3 * natom);
    vector geoc(3 * natom);
    vector omega(3 * natom - 6);
    vector omegaion(3 * natom - 6);
    matrix Lmat(3 * natom, 3 * natom - 6);
    matrix Lmation(3 * natom, 3 * natom - 6);
    matrix FCMn(3 * natom, 3 * natom);
    matrix FCMc(3 * natom, 3 * natom);
    ...
    for (i = 0; i < 2; i++) {
      ...
      get geometries */
        ...
        if (usingnormalmodes == 0) {
            ...
            if (i == 0) {
                for (l = 0; l < (3 * natom) / 4; l++) {
                    for (lp = 0; lp < (3 * natom); lp++) {
                        for (j = 0; j < 4; j++) {
                            file >> FCMn(lp, j + l * 4);
// 	      std::cout.width(12);std::cout.precision(6);
// 	      std::cout<<FCMn(lp,j+l*4)<<" ";
                        }
                        //std::cout<<std::endl;
                    }
                    //std::cout<<std::endl;
                ...
                for (lp = 0; lp < (3 * natom); lp++) {
                    for (j = 0; j < (3 * natom) % 4; j++) {
                        file >> FCMn(lp, (3 * natom) - 1 + j);
// 	    std::cout<<FCMn(lp,(3*natom)-1+j)<<" ";
// 	    std::cout<<std::endl;
                    }
                }
            }
            if (i == 1) {
                ...
                for (lp = 0; lp < (3 * natom); lp++) {
                    for (j = 0; j < (3 * natom) % 4; j++) {
                        file >> FCMc(lp, (3 * natom) - 1 + j);
                        // 	    std::cout<<FCMc(lp,(3*natom)-1+j)<<" ";
                        // 	    std::cout<<std::endl;
                    }
                }
            }
...
//   matrix Rwhole=Rx*Ry*Rz;

    //rotate the geometry
//   int ip,jp;
//   matrix RotateCartesian(3*natom,3*natom);
//   matrix RotateCartesianMW(3*natom,3*natom);
//   for (i=0;i<natom;i++)
//     for (ip=0;ip<3;ip++)
//       for (jp=0;jp<3;jp++) {
// 	RotateCartesian(i*3+ip,i*3+jp)=Rwhole(ip,jp);
// 	RotateCartesianMW(i*3+ip,i*3+jp)=Rwhole(ip,jp);
//       }

//   geoc=RotateCartesian*geoc;
//   Lmation=RotateCartesianMW*Lmation;
    ...
    /*	vector Rvec=geon-geoc; //makes more sense */
    /* uncomment the foolowing line for geometries in Angstrom */
    if (Units == 'A') Rvec = (1. / .529177) * Rvec;
    if (Units != 'A' && Units != 'B') std::cerr << "wrong Units flag" << std::endl;
    /*		Rvec=geoc-geonfix;
		no difference under translation */
    ...
    if (usingnormalmodes != 0) {
        ...
        vibreportout << "mode";
        vibreportout << " initial state (1/CM)";
        vibreportout << " final state (1/CM)" << std::endl;
        for (k = 0; k < (3 * natom); k++)
            vibreportout << k << " " << sqrt(complex(NMn(k), 0.)) * hatocm << " " << sqrt(complex(NMc(k), 0.)) * hatocm
                         << std::endl;
        // inertia tensor
        matrix In(3, 3);
        for (i = 0; i < natom; i++) {
            In(0, 0) += massvec(i) * (geon(i * 3 + 1) * geon(i * 3 + 1) + geon(i * 3 + 2) * geon(i * 3 + 2));
            In(1, 1) += massvec(i) * (geon(i * 3 + 0) * geon(i * 3 + 0) + geon(i * 3 + 2) * geon(i * 3 + 2));
            In(2, 2) += massvec(i) * (geon(i * 3 + 1) * geon(i * 3 + 1) + geon(i * 3 + 0) * geon(i * 3 + 0));
            In(0, 1) += -massvec(i) * (geon(i * 3 + 0) * geon(i * 3 + 1));
            In(0, 2) += -massvec(i) * (geon(i * 3 + 0) * geon(i * 3 + 2));
            In(1, 2) += -massvec(i) * (geon(i * 3 + 1) * geon(i * 3 + 2));
        }
        In(1, 0) = In(0, 1);
        In(2, 0) = In(0, 2);
        In(2, 1) = In(1, 2);
        ...
        vector D6(3 * natom);
        for (i = 0; i < natom; i++) {
            D1(i * 3 + 0) = sqrt(massvec(i));
            D2(i * 3 + 1) = sqrt(massvec(i));
            D3(i * 3 + 2) = sqrt(massvec(i));
        }
        // D4j;i = ((Py)i Xj;3 -  (Pz)i Xj;2) sqrt(mi)
        // D5j;i = ((Pz)i Xj;1 -  (Px)i Xj;3) sqrt(mi)
        // D6j;i = ((Px)i Xj;2 -  (Py)i Xj;1) sqrt(mi)
        /* where j = x; y; z; i is over all atoms and P is the dot product of R
       (the coordinates of the atoms with respect to the center of mass)
       and the corresponding row of X, the matrix used to diagonalize the
       moment of inertia tensor I.
    */
        vector Pvec(natom * 3);
        for (i = 0; i < natom; i++) {
            for (j = 0; j < 3; j++) {
                Pvec(i * 3 + 0) += geon(i * 3 + j) * In(j, 0);
                Pvec(i * 3 + 1) += geon(i * 3 + j) * In(j, 1);
                Pvec(i * 3 + 2) += geon(i * 3 + j) * In(j, 2);
            }
        }
        for (i = 0; i < natom; i++) {
            D4(i * 3 + 0) = (Pvec(i * 3 + 1) * In(0, 2) - Pvec(i * 3 + 2) * In(0, 1)) * sqrt(massvec(i));
            D4(i * 3 + 1) = (Pvec(i * 3 + 1) * In(1, 2) - Pvec(i * 3 + 2) * In(1, 1)) * sqrt(massvec(i));
            D4(i * 3 + 2) = (Pvec(i * 3 + 1) * In(2, 2) - Pvec(i * 3 + 2) * In(2, 1)) * sqrt(massvec(i));

            D5(i * 3 + 0) = (Pvec(i * 3 + 2) * In(0, 0) - Pvec(i * 3 + 0) * In(0, 2)) * sqrt(massvec(i));
            D5(i * 3 + 1) = (Pvec(i * 3 + 2) * In(1, 0) - Pvec(i * 3 + 0) * In(1, 2)) * sqrt(massvec(i));
            D5(i * 3 + 2) = (Pvec(i * 3 + 2) * In(2, 0) - Pvec(i * 3 + 0) * In(2, 2)) * sqrt(massvec(i));

            D6(i * 3 + 0) = (Pvec(i * 3 + 0) * In(0, 1) - Pvec(i * 3 + 1) * In(0, 0)) * sqrt(massvec(i));
            D6(i * 3 + 1) = (Pvec(i * 3 + 0) * In(1, 1) - Pvec(i * 3 + 1) * In(1, 0)) * sqrt(massvec(i));
            D6(i * 3 + 2) = (Pvec(i * 3 + 0) * In(2, 1) - Pvec(i * 3 + 1) * In(2, 0)) * sqrt(massvec(i));
        }
        normalise(D1);
        normalise(D2);
        normalise(D3);
        ...
        normalise(D5);
        normalise(D6);
        std::cout << D4 * D5 << " " << D5 * D6 << " " << D4 * D6 << std::endl;

        for (k = 0; k < (3 * natom - 6); k++) {
            omegaion(k) = sqrt(NMc(k + 6));
            omega(k) = sqrt(NMn(k + 6));
            //std::cout<<omega(k)*hatocm<<" "<<omegaion(k)*hatocm<<std::endl;
            for (j = 0; j < (3 * natom); j++) {
                Lmat(j, k) = mwHn(j, k + 6);
                Lmation(j, k) = mwHc(j, k + 6);
            }
        }

        // phase correction
        matrix Stest = transpose(Lmation) * Lmat;
        for (k = 0; k < (3 * natom - 6); k++) {
            if (Stest(k, k) < 0) {
                for (j = 0; j < (3 * natom); j++)
                    Lmation(j, k) = -mwHc(j, k + 6);
    ...
    for (k = 0; k < (3 * natom - 6); k++) {
        ...
        proj2 = sqrt(proj2);
        for (i = 0; i < (3 * natom); i++) {
            oLmat(i, k) = oLmat(i, k) / proj1;
            oLmation(i, k) = oLmation(i, k) / proj2;
        }
        for (j = k + 1; j < (3 * natom - 6); j++) {
            proj1 = 0.0;
            proj2 = 0.0;
            for (i = 0; i < (3 * natom); i++) {
...
//   matrix projectorion=Lmation*transpose(Lmation);
//   matrix projector=Lmat*transpose(Lmat);
//   matrix projectorU=projector;
//   matrix projectorionU=projectorion;

//   vector singval=diag(projectorU);
//   vector singvalion=diag(projectorionU);

//   matrix nonsingularvectors(3*natom,N);
//   matrix nonsingularvectorsion(3*natom,N);
//   diagmat inversesingvalion(N);
//   diagmat inversesingval(N);
//   for (i=0;i<N;i++) {
//     inversesingvalion(i)=1./singvalion(6+i);
//     inversesingval(i)=1./singval(6+i);
//   }
//   for (i=0;i<3*natom;i++) {
//     for (j=0;j<N;j++) {
//       nonsingularvectors(i,j)=projectorU(i,j+6);
// 	nonsingularvectorsion(i,j)=projectorionU(i,j+6);
//     }
//   }

//   matrix inverseprojectorion=
//     nonsingularvectorsion*inversesingvalion*transpose(nonsingularvectorsion);
//   matrix inverseprojector=
//     nonsingularvectors*inversesingval*transpose(nonsingularvectors);

//   matrix inverseofLmation=transpose(Lmation)*inverseprojectorion;
//   matrix inverseofLmat=transpose(Lmat)*inverseprojector;

//   if (inverseofL ==1) {
//     inverseofLmation=transpose(Lmation);
//     inverseofLmat=transpose(Lmat);
//   }

    diagmat lambda(3 * natom - 6);
    diagmat lambdaion(3 * natom - 6);
    for (j = 0; j < (3 * natom - 6); j++) {
        lambda(j) = sqrt(omega(j));
        lambdaion(j) = sqrt(omegaion(j));
    }
    matrix OSmat = transpose(Lmation) * Lmat;
    //matrix OSmat=inverseofLmation*Lmat;
    matrix Smat = OSmat;
...
;
    ...
    //matrix Jmat=inverseofLmat*Lmation;
    vector K = transpose(Lmat) * sqmass * (geoc - geon);
    //vector K=inverseofLmat*sqmass*(geoc-geon);

    diagmat II(3 * natom - 6);
    for (j = 0; j < (3 * natom - 6); j++) II(j) = 1.;
    matrix JJ = lambdaion * (Smat * inverse(lambda));
...
;
    ...
    int00 = pow(int00, .25);
    int00 *= exp(log(sqrt(det(Qmat))) + ((double) ((3 * natom - 6)) / 2.) * log(2.));
    int00 *= exp(-.5 * delta * ((II - Pmat) * delta));
    ...
    fcfile.precision(3);
    fcfile.setf(std::ios::fixed, std::ios::floatfield);
    ...
    for (i = 0; i < size; i++) {
        specf.precision(3);
        specf.setf(std::ios::fixed, std::ios::floatfield);
        specf << grid(i) << "  ";
        specf.precision(6);
        specf.setf(std::ios::scientific, std::ios::floatfield);
        if ((sumlor(i)) >= snlimit)
    ...
    stat << "frequency list" << std::endl;
    stat << "mode  neutral  ion" << std::endl;
    stat << " number of bands calculated: " << fccount << std::endl;
    stat.precision(4);
    double ZPEn = 0.;
    double ZPEc = 0.;
    for (i = 0; i < (3 * natom - 6); i++) {
        stat << i + 1 << "      " << omega(i) * hatocm << "     " << omegaion(i) * hatocm << std::endl;
        ZPEn += omega(i);
        ZPEc += omegaion(i);
    }
    ZPEn *= (.5 * hatocm);
    ZPEc *= (.5 * hatocm);
    stat << "ZPE neutral= " << ZPEn << std::endl;
    stat << "ZPE cation= " << ZPEc << std::endl;
}
...
/*	if (n==2) return (complex(-2.,0.)+ complex(4.,0.)*pow(x,2.));
	if (n==3) return (complex(-12.,0.)*x + complex(8.,0.)*pow(x,3.));
	if (n==4) return (complex(12.,0.)- complex(48.,0.)*pow(x,2.)
	+complex(16.,0.)*pow(x,4.));
	if (n==5) return (complex(120.,0.)*x-complex(160.,0.)*pow(x,3.)
	+complex(32.,0.)*pow(x,5.));
	if (n==6) return (complex(-120.,0.)+complex(720.,0.)*pow(x,2.)
	-complex(480.,0.)*pow(x,4.) +
	complex(64.,0.)*pow(x,6.));
	if (n==7) return (complex(-1680.,0.)*x + complex(3360.,0.)*pow(x,3.)
	- complex(1344.,0.)*pow(x,5.) + complex(128.,0.)*pow(x,7.));
	*/
    complex val;
